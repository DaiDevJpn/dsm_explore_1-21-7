...
public class BlockCrashEvent {
    ...
    public static void onBlockBroken(BlockEvent.BreakEvent event) {
        ...
        CrashRecipe recipe = manager.find(held.getItem(), state);
        ...
        if (recipe.baseOutput().chance() >= rand.nextFloat()){
            drops.add(new ItemEntity(level, center.x, center.y, center.z, dropB));
        }

        for (DropEntry d : recipe.additionalOutputs()) {
            ItemStack dropA = d.createStack(rand);
            if (d.chance() >= rand.nextFloat()){
                drops.add(new ItemEntity(level, center.x, center.y, center.z, dropA));
            }
        }
